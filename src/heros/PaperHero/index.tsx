...
export const PaperHero: React.FC<PaperHeroProps> = ({ eyebrow, media, paperSettings, richText }) => {
  ...
  return (
    <div
      className="relative min-h-screen overflow-hidden flex flex-col items-center justify-center bg-rtm-ground-hero text-rtm-fg mt-[calc(var(--header-height)*-1)] px-[4vw]"
    >
      ...
      <div className="relative z-[2] flex flex-col items-center gap-[clamp(16px,2vw,24px)] text-center max-w-[90vw]">
        {eyebrow && (
          <span
            className="text-rtm-label font-rtm-mono-label tracking-label uppercase text-rtm-fg"
            style={{ textShadow: '0 0 40px var(--rtm-ground-hero)' }}
          >
            {eyebrow}
          </span>
        )}
        ...
        {richText && (
          <RichText
            className="[&_h1]:m-0 [&_h1]:font-rtm-display [&_h1]:font-black [&_h1]:uppercase [&_h1]:text-[13vw] max-sm:[&_h1]:text-[18vw] [&_h1]:leading-[0.85] [&_h1]:tracking-[-0.04em] [&_h1]:text-rtm-bg [&_h1]:mix-blend-difference [&_p]:m-0 [&_p]:mt-2 [&_p]:font-rtm-serif [&_p]:italic [&_p]:text-[clamp(17px,1.6vw,22px)] [&_p]:leading-[1.5] [&_p]:text-rtm-fg [&_p]:max-w-[52ch] [&_p]:text-pretty [&_p]:[text-shadow:0_0_40px_var(--rtm-ground-hero)] grid justify-items-center gap-[clamp(16px,2vw,24px)]"
            data={richText}
            enableGutter={false}
            enableProse={false}
          />
      ...
      {paperSettings?.metaLine && (
        <div
          className="absolute z-[2] right-10 bottom-10 max-sm:right-5 max-sm:bottom-5 max-w-[300px] text-right text-rtm-caption font-rtm-mono-label uppercase text-rtm-fg"
          style={{ textShadow: '0 0 40px var(--rtm-ground-hero)' }}
        >
          {paperSettings.metaLine}
        </div>
